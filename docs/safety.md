...
Worktree-local Git metadata is restored into the directory reported by `git -C <worktree> rev-parse --absolute-git-dir`, not left accessible only through `.bare`. This includes `logs/HEAD`, `ORIG_HEAD`, `FETCH_HEAD`, other pseudorefs and state files, and the `refs/bisect`, `refs/worktree`, and `refs/rewritten` namespaces with their reflogs. Private refs are removed from the copied common ref store, including `packed-refs`, before any linked worktrees are created. They are then materialized through Git only in the migrated worktree. Verification rejects private refs left in the common database so existing and future worktrees cannot resolve them. Shared metadata stays in `.bare`; split directories retain their permissions and extended attributes in both locations. Source metadata is checked for changes before promotion, and destination metadata and refs are verified through the migrated worktree both before and after promotion.

Configuration includes must keep their meaning after relocation. Relative includes from repository config files that escape `.git`, and absolute includes pointing back into the moving repository, are refused before preparation. Keep included settings inside `.git` with relative include paths, or use stable absolute paths outside the repository. Nested includes are inspected through Git; stable external includes and one-off include overrides remain supported. Conditional `includeIf` rules are not rewritten. Effective configuration and remote names must match at both the repository root and migrated worktree before and after promotion, except for required bare/relative-worktree settings, normalized local remote URLs, and Git's tracking setup for a newly created default branch. A mismatch after promotion triggers rollback. Diagnostics name changed keys without printing their values, and source configuration is checked again for concurrent changes.
