...
func TestParsePorcelainKeepsFullIDsAndNonBareNamedBare(t *testing.T) {
	head := strings.Repeat("a", 64)
	entries := ParsePorcelain("worktree /repo/.bare\x00HEAD " + head + "\x00branch refs/heads/main\x00locked  reason \x00\x00")
	if len(entries) != 1 || entries[0].Head != head || entries[0].LockedReason != " reason " {
