...
func migrationPrivateRef(ref string) bool {
	return strings.HasPrefix(ref, "refs/worktree/") || strings.HasPrefix(ref, "refs/bisect/") || strings.HasPrefix(ref, "refs/rewritten/")
}
...
func removeMigrationCommonPrivateRefs(ctx context.Context, plan migratePlan, root string) error {
	...
	for _, line := range strings.Split(plan.refs, "\n") {
		...
		if _, err := git.RunInWithOutputContext(ctx, root, "-c", "core.hooksPath=/dev/null", "update-ref", "--no-deref", "-d", fields[0]); err != nil {
			return err
		}
	}
